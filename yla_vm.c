/*
	...
	yla_vm_get_value: комментарий к функции.
	Функция yla_vm_get_value_internal() не считывает переменную 
	с того места, на котором она фактически находится,
	если использовать vm->code в качестве 
	входного параметра данной функции.
	В этом можно легко убедиться, заменив в строке:
	*value = yla_vm_get_value_internal(prog_var);
	входной параметр 'prog_var' на 'vm->code'.
	Функция считает лишь первые 2 шестнадцатеричных числа, 
	находящихся в параметре 'vm->code'
	(данный параметр хранит в себе весь код входной программы,
	исключая заголовок).
	
	Именно поэтому prog_var[2] хранит в себе эту переменную, 
	чтоб считать её в функции get_value_internal.
